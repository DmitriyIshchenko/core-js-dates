...
function getNextFriday(date) {
  ...
  day += weekday < FRIDAY ? FRIDAY - weekday : WEEK_LENGTH - (weekday - FRIDAY);
...
// take advantage of 0-based months and automatically fixed dates
function getCountDaysInMonth(month, year) {
  const nextMonth = new Date(year, month);
  const lastDay = new Date(nextMonth.setDate(0));
...
function isDateInPeriod(date, period) {
  ...
  return (
    targetDate >= new Date(period.start) && targetDate <= new Date(period.end)
  );
}
...
function formatDate(str) {
  ...
  return `${month}/${day}/${year}, ${formattedHours}:${minutes}:${seconds} ${postfix}`;
}
...
function getCountWeekendsInMonth(month, year) {
  return Array.from(
    ...
    (_, index) => new Date(year, month - 1, index + 1).getDay()
  ).filter((day) => day % 6 === 0).length;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const format = (date) =>
    new Intl.DateTimeFormat('en-GB').format(date).replaceAll('/', '-');

  let isWorking = true;
  const currentDate = initDate(period.start);
  const endDate = initDate(period.end);
  const schedule = [];

  while (currentDate <= endDate) {
    const countDays = isWorking ? countWorkDays : countOffDays;

    for (let i = 0; i < countDays; i += 1) {
      if (currentDate > endDate) break;

      if (isWorking) schedule.push(format(currentDate));
